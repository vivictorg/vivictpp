...
void requireSettingsEquals(const vivictpp::Settings &lhs,
                           const vivictpp::Settings &rhs);

TEST_CASE("Empty settings file"
          "[Settings]") {
  vivictpp::Settings settings =
      vivictpp::loadSettings("../testdata/settings/empty_settings.toml");
  vivictpp::Settings expectedSettings;
  requireSettingsEquals(settings, expectedSettings);
}

TEST_CASE("Incomplete settings file"
          "[Settings]") {
  vivictpp::Settings settings =
      vivictpp::loadSettings("../testdata/settings/incomplete_settings.toml");
  vivictpp::Settings expectedSettings;
  ...
  expectedSettings.logLevels = {{"SeekState", "warn"}, {"RandomLog", "error"}};
  requireSettingsEquals(settings, expectedSettings);
}

TEST_CASE("Complete settings file"
          "[Settings]") {
  vivictpp::Settings expectedSettings;
  ...
  expectedSettings.logLevels = {{"SeekState", "warn"}, {"RandomLog", "error"}};
  vivictpp::Settings settings =
      vivictpp::loadSettings("../testdata/settings/complete_settings.toml");
  requireSettingsEquals(settings, expectedSettings);
}
