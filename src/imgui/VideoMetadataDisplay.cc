...
float vivictpp::imgui::VideoMetadataDisplay::calcWidth() {
  return vivictpp::ui::FontSize::getScaleFactor() *
         (FIRST_COLUMN_WIDTH + SECOND_COLUMN_WIDTH);
}

void lineHere(float x0, float x1) {
  float y = ImGui::GetCursorPosY() + ImGui::GetTextLineHeight();
  ImGui::GetWindowDrawList()->AddLine({x0, y}, {x1, y},
                                      vivictpp::imgui::border);
}

void vivictpp::imgui::VideoMetadataDisplay::draw(
    const ui::DisplayState &displayState) {
  const VideoMetadata &metadata = (type == Type::LEFT)
                                      ? displayState.leftVideoMetadata
                                      : displayState.rightVideoMetadata;
  if (metadata.empty())
    return;
  const libav::DecoderMetadata decoderMetadata =
      (type == Type::LEFT) ? displayState.leftDecoderMetadata
                           : displayState.rightDecoderMetadata;
  float width = calcWidth();
  float scaling = vivictpp::ui::FontSize::getScaleFactor();

  p1 = {ImGui::GetCursorPosX() - PAD,
  ...
  vivictpp::imgui::tableRow2(cw, "resolution", "%s",
                             metadata.filteredResolution.toString());
  vivictpp::imgui::tableRow2(cw, "sample ar", "%d:%d",
                             metadata.filteredSampleAspectRatio.num,
                             metadata.filteredSampleAspectRatio.den);
  if (metadata.filteredResolution != metadata.resolution) {
    vivictpp::imgui::tableRow2(cw, "orig resolution", "%s",
                               metadata.resolution.toString());
  }
  if (av_cmp_q(metadata.sampleAspectRatio,
               metadata.filteredSampleAspectRatio) != 0) {
    vivictpp::imgui::tableRow2(cw, "orig sample ar", "%d:%d",
                               metadata.sampleAspectRatio.num,
                               metadata.sampleAspectRatio.den);
  }
  ...
  vivictpp::imgui::tableRow2(cw, "duration", "%s",
                             vivictpp::time::formatTime(metadata.duration));
  vivictpp::imgui::tableRow2(cw, "start time", "%s",
                             vivictpp::time::formatTime(metadata.startTime));
  ImGui::Dummy({2.0f, 1.0f});
  vivictpp::imgui::tableRow2(cw, "decoder", "%s", decoderMetadata.name);
  vivictpp::imgui::tableRow2(cw, "hardware acceleration", "%s",
                             decoderMetadata.hwAccel);
  if (decoderMetadata.hwAccel != "none") {
    vivictpp::imgui::tableRow2(cw, "hardware pix fmt", "%s",
                               decoderMetadata.hwPixelFormat);
  }
  if (!displayState.isPlaying) {
    ImGui::Dummy({2.0f, 1.0f});
    const FrameMetadata &frameMetadata =
        (type == Type::LEFT) ? displayState.leftFrame.metadata()
                             : displayState.rightFrame.metadata();
    vivictpp::imgui::tableRow2(cw, "Frame type", "%c",
                               frameMetadata.pictureType);
