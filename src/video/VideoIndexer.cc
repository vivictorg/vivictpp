...
void pairsort(std::vector<float> &a, std::vector<float> &b, int n)
{
 ...
 void vivictpp::video::VideoIndex::addFrameData(const IndexFrameData &frameData) {
  ...
  void vivictpp::video::VideoIndex::addThumbnail(const vivictpp::video::Thumbnail &thumbnail) {
    std::lock_guard<std::mutex> lg(m);
  ...
  void vivictpp::video::VideoIndex::finalizeIndex() {
    ...
    addGop(end);
    pairsort(plotDatas.frameSize.pts, plotDatas.frameSize.values, plotDatas.frameSize.pts.size());
    indexingDone = true;
  }
