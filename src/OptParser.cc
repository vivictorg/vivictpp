...
Vivict++ )" +
    std::string(VPP_VERSION) + " (" + std::string(VPP_GIT_HASH) + ")";
...
bool vivictpp::OptParser::parseOptions(int argc, char **argv) {
  ...
  app.add_option("rightVideo", rightVideo,
                 "Path or url to second (right) video");
  ...
  app.add_option("--left-format", leftInputFormat,
                 "Format options for left video input");
  app.add_option("--right-format", rightInputFormat,
                 "Format options for right video input");

  std::string hwAccel("none");
  app.add_option("--hwaccel", hwAccel,
                 std::string("Select device type to use for hardware "
                             "accelerated decoding. Valid values are:\n") +
                     "  auto    Use any available device type (default)\n" +
                     "  none    Disable hardware accelerated decoding\n" +
                     "  TYPE    Name of devicetype, see "
                     "https://trac.ffmpeg.org/wiki/HWAccelIntro");

  std::string preferredDecodersStr;
  app.add_option("--preferred-decoders", preferredDecodersStr,
                 std::string("Comma separated list of decoders that should be "
                             "preferred over default decoder when applicable"));
  ...
  std::vector<std::string> preferredDecoders =
      splitString(preferredDecodersStr);
  ...
  for (size_t i = 0; i < sources.size(); i++) {
    ...
    //        std::string vmafLogFile = i < vmafLogfiles.size() ?
    //        vmafLogfiles[i] : "";
