...
vivictpp::ui::YuvRenderer::YuvRenderer(const VideoMetadata &videoMetadata,
...
void vivictpp::ui::YuvRenderer::render(glm::mat4 &MVP, GLuint vertexbuffer,
                                       GLuint uvbuffer, GLubyte **planes) {
...
bool vivictpp::ui::OpenGLVideoTextures::update(
...
bool vivictpp::ui::OpenGLVideoTextures::initTextures(
    const vivictpp::ui::DisplayState &displayState) {
  if (videoMetadataVersion == displayState.videoMetadataVersion)
    return false;
  videoTextures.calcNativeResolution(displayState);
  bool render10bit = true;
  leftTextureRenderer = vivictpp::ui::opengl::createTextureRenderer(
      displayState.leftVideoMetadata, GL_TEXTURE0, render10bit);
  videoTextures.leftTexture =
      (ImTextureID)((void *)leftTextureRenderer->getRenderedTextureId());
  if (!displayState.rightVideoMetadata.empty()) {
    rightTextureRenderer = vivictpp::ui::opengl::createTextureRenderer(
        displayState.rightVideoMetadata,
        GL_TEXTURE0 + leftTextureRenderer->getTextureUnitCount(), render10bit);
    videoTextures.rightTexture =
        (ImTextureID)((void *)rightTextureRenderer->getRenderedTextureId());
  }
  return true;
}
