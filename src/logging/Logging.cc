...
void avLogCallback(void *avcl,
                   int level,
                   const char *fmt,
                   va_list vl) {
    char line[avLogLineSize];
    static int printPrefix = 1;
    static vivictpp::logging::Logger logger = vivictpp::logging::getOrCreateLogger("libav");
    std::lock_guard<std::mutex> lock(avLogMutex);
    if (level > avLogLevel) {
        return;
    }
    int n = av_log_format_line2(avcl, level, fmt, vl, line, avLogLineSize, &printPrefix);
    if (n > 1) {
        // Remove newline
    ...
    const spdlog::level::level_enum spdLogLevel = avLogLevelToSpdLogLevel.at(level);
...
const std::map<std::string, spdlog::level::level_enum> &vivictpp::logging::getLogLevels() {
  return logLevels;
}
...
void vivictpp::logging::setLogLevels(const std::map<std::string, std::string> &newLevels, bool createLoggers) {
    spdlog::level::level_enum defaultLevel = logLevels.at(newLevels.at(DEFAULT_LOGGER_NAME));
    ...
    for (auto l : newLevels) {
        ...
        if (loggerName == LIBAV_LOGGER_NAME) {
            std::lock_guard<std::mutex> lock(avLogMutex);
            spdlog::level::level_enum level = levelName == "(default)" ? defaultLevel : logLevels.at(levelName);
            avLogLevel = spdLogLevelToAvLogLevel.at(level);
        }
        Logger logger = createLoggers ? getOrCreateLogger(loggerName) : spdlog::get(loggerName);
        if (!logger) {
            continue;
        }
        spdlog::level::level_enum level = levelName == "(default)" ? defaultLevel : logLevels.at(levelName);
...
void initSinks(const vivictpp::Settings &settings) {
    ringbufferSink = std::make_shared<spdlog::sinks::ringbuffer_sink_mt>(settings.logBufferSize);
    sinks.push_back(ringbufferSink);
    if (settings.logToFile) {
        auto fileSink = std::make_shared<spdlog::sinks::basic_file_sink_mt>(settings.logFile);
