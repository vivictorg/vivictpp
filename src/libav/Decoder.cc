...
vivictpp::libav::Decoder::Decoder(AVCodecParameters *codecParameters,
    ...
    : codecContext(nullptr),
      ...
      hwDeviceContext(nullptr), hwPixelFormat(AV_PIX_FMT_NONE),
...
void vivictpp::libav::Decoder::initCodecContext(
    AVCodecParameters *codecParameters, const DecoderOptions &decoderOptions) {
  AVCodecID codecId = codecParameters->codec_id;
  const AVCodec *codec = findDecoder(codecId, decoderOptions);
  if (codec == nullptr) {
    throw std::runtime_error(std::string("No codec found for codec ID: ") +
...
void logSupportedDeviceTypes() {
  auto logger =
      vivictpp::logging::getOrCreateLogger("vivictpp::libav::Decoder");
...
void vivictpp::libav::Decoder::initHardwareContext(
  ...
  for (auto &type : candidateTypes) {
    for (size_t i = 0;; i++) {
      const AVCodecHWConfig *config =
          avcodec_get_hw_config(codecContext->codec, i);
      if (!config) {
        break;
      }
      if (config->methods & AV_CODEC_HW_CONFIG_METHOD_HW_DEVICE_CTX &&
        ...
        vivictpp::libav::AVResult ret =
            av_hwdevice_ctx_create(&hwDeviceContext, type, NULL, NULL, 0);
        if (ret.error()) {
          logger->info("Failed to create hardware device of type {}",
          ...
          this->decoderMetadata.hwPixelFormat =
          ...
          this->codecContext->hw_device_ctx =
              av_buffer_ref(this->hwDeviceContext.get());
  ...
  logger->info("No hardware device found for codec {}",
               this->codecContext.get()->codec->name);
}
...
#endif
  vivictpp::libav::AVResult ret =
      avcodec_open2(codecContext.get(), codecContext->codec, &decoderOptions);
...
void vivictpp::libav::Decoder::logAudioCodecInfo() {
  if (codecContext->codec_type == AVMEDIA_TYPE_AUDIO) {
    ...
    logger->info("Audio Codec Context: sample_rate={}, channels={}, "
                 "channel_layout={}, format={}",
                 codecContext->sample_rate, channels, channelLayout,
                 std::string(av_get_sample_fmt_name(codecContext->sample_fmt)));
  }
}

void vivictpp::libav::Decoder::flush() {
  avcodec_flush_buffers(this->codecContext.get());
}

std::vector<vivictpp::libav::Frame>
vivictpp::libav::Decoder::handlePacket(Packet packet) {
  logger->trace("handlePacket");
  vivictpp::libav::AVResult ret =
      avcodec_send_packet(this->codecContext.get(), packet.avPacket());
  if (ret.error() && !ret.eof()) {
    throw std::runtime_error(std::string("Send packet failed: ") +
                             ret.getMessage());
  }
  std::vector<Frame> result;
  while ((ret = avcodec_receive_frame(this->codecContext.get(),
                                      nextFrame.avFrame()))
             .success()) {
  ...
  throw std::runtime_error(std::string("Receive frame failed: ") +
                           ret.getMessage());
}

static const std::vector<AVPixelFormat> preferredPixelFormats = {
    AV_PIX_FMT_YUV420P, AV_PIX_FMT_YUV420P10, AV_PIX_FMT_NV12, AV_PIX_FMT_P010};
...
void vivictpp::libav::Decoder::selectSwPixelFormat() {
  ...
  av_hwframe_transfer_get_formats(nextFrame.avFrame()->hw_frames_ctx,
                                  AV_HWFRAME_TRANSFER_DIRECTION_FROM,
                                  &pixelFormats, 0);

  for (enum AVPixelFormat *curr = pixelFormats; *curr != AV_PIX_FMT_NONE;
       curr++) {
  ...
  for (const auto &format : preferredPixelFormats) {
    if (std::find(availableTargetFormats.begin(), availableTargetFormats.end(),
                  format) != availableTargetFormats.end()) {
