...
struct DisplayState {
  ...
  void updateFrames(std::array<vivictpp::libav::Frame, 2> frames) {
    ...
    if (leftFrame.hasUpdatedFilteredMetadata()) {
      leftVideoMetadata = VideoMetadata(
          leftVideoMetadata, leftFrame.getAndResetUpdatedFilteredMetadata());
      videoMetadataVersion++;
    }
    if (rightFrame.hasUpdatedFilteredMetadata()) {
      rightVideoMetadata = VideoMetadata(
          rightVideoMetadata, rightFrame.getAndResetUpdatedFilteredMetadata());
      videoMetadataVersion++;
    }
  };
